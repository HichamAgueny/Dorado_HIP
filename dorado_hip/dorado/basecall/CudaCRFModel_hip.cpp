...
public:
    CudaCaller(const CRFModelConfig &model_config,
        ...
        c10::hip::HIPGuardMasqueradingAsCUDA device_guard(m_options.device());
    ...
    std::vector<decode::DecodedChunk> call_chunks(at::Tensor &input,
        ...
        NVTX3_FUNC_RANGE();
        c10::hip::HIPStreamGuardMasqueradingAsCUDA stream_guard(stream);
    ...
    void cuda_thread_fn() {
        at::InferenceMode guard;
        c10::hip::HIPGuardMasqueradingAsCUDA device_guard(m_options.device());
        auto stream = c10::hip::getCurrentHIPStreamMasqueradingAsCUDA(m_options.device().index());
        ...
        while (true) {
            ...
            auto device_stats =
                    c10::hip::HIPCachingAllocator::getDeviceStats(m_options.device().index());

            auto print_stat = [](c10::hip::HIPCachingAllocator::StatArray &st) {
                std::string s("");
...
CudaModelRunner::CudaModelRunner(std::shared_ptr<CudaCaller> caller)
        : m_caller(caller),
          m_stream(c10::hip::getStreamFromPoolMasqueradingAsCUDA(false, m_caller->m_options.device().index())) {
