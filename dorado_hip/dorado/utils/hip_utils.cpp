...
MatmulMode get_cuda_matmul_fp16_mode() {
    ...
    if (env_matmul_fp16_mode != nullptr) {
        ...
        if (matmul_fp16_mode_str == "TORCH") {
            ...
            spdlog::debug(">   Using rocblas_gemmex");
            return MatmulMode::CUBLAS;
        }
        spdlog::debug(">   Ignoring unrecognized option. Select from TORCH or CUBLAS.");
    }

    // torch::matmul() is a bit slower than rocblas_gemmex() on A100 and V100, and 2x slower on TX2
...
void print_cuda_alloc_info(const std::string &label) {
    auto stats = c10::hip::HIPCachingAllocator::getDeviceStats(0);
    auto print_stat_array = [](c10::hip::HIPCachingAllocator::StatArray &stat,
...
size_t available_memory(torch::Device device) {
    size_t free, total;
    c10::hip::HIPGuardMasqueradingAsCUDA device_guard(device);
...
void matmul_f16_cublas(const at::Tensor &A, const at::Tensor &B, at::Tensor &C) {
    ...
    auto res = rocblas_gemmex(at::cuda::getCurrentCUDABlasHandle(), rocblas_operation_none, rocblas_operation_none,
                            int(B.size(1)), int(A.size(0)), int(A.size(1)), &HALF_ONE, B.data_ptr(),
                            HIP_R_16F, int(B.stride(0)), A.data_ptr(), HIP_R_16F,
                            int(A.stride(0)), &HALF_ZERO, C.data_ptr(), HIP_R_16F,
                            int(C.stride(0)), HIP_R_16F, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
