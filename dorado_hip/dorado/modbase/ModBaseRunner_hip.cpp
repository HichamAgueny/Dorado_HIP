...
public:
    ...
    at::Tensor call_chunks(size_t model_id,
        ...
        if (m_options.device().is_cuda()) {
            task->stream = c10::hip::getCurrentHIPStreamMasqueradingAsCUDA(m_options.device().index());
        }
#endif
        {
    ...
    void modbase_task_thread_fn(size_t model_id) {
        ...
        while (true) {
            ...
            c10::hip::OptionalHIPStreamGuardMasqueradingAsCUDA stream_guard(task->stream);
#endif
...
ModBaseRunner::ModBaseRunner(std::shared_ptr<ModBaseCaller> caller) : m_caller(std::move(caller)) {
    ...
    for (auto& caller_data : m_caller->m_caller_data) {
        ...
        if (m_caller->m_options.device().is_cuda()) {
            m_streams.push_back(
                    c10::hip::getStreamFromPoolMasqueradingAsCUDA(false, m_caller->m_options.device().index()));
        } else {
...
#if DORADO_GPU_BUILD && !defined(__APPLE__)
    c10::hip::OptionalHIPStreamGuardMasqueradingAsCUDA guard(m_streams[model_id]);
#endif
